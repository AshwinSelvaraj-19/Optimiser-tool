...
logger = get_logger("emulator.ldplayer")


class LDPlayer(BaseEmulator):
    """LDPlayer emulator implementation."""
    ...
    def get_install_paths(self) -> list:
        ...
        return [
            f"{program_files}\\leidian\\LDPlayer9",
            f"{program_files_x86}\\leidian\\LDPlayer9",
            f"{program_files}\\leidian\\LDPlayer4",
            f"{program_files_x86}\\leidian\\LDPlayer4",
    ...
    def detect(self) -> bool:
        success, stdout, _ = run_powershell(
            ...
            'Where-Object { $_.DisplayName -like "*LDPlayer*" } | '
    ...
    def read_config(self) -> Optional[EmulatorConfig]:
        ...
        if self._info.install_path:
            conf_path = os.path.join(self._info.install_path, "vms", "config", "leidian.cfg")
            if os.path.exists(conf_path):
                try:
                    import json
                    with open(conf_path, "r", encoding="utf-8", errors="replace") as f:
                        config.raw_settings = json.load(f)
                except Exception as e:
                    logger.debug(f"LDPlayer config parse error: {e}")
        return config

    def write_config(self, config: EmulatorConfig) -> bool:
        logger.debug("LDPlayer config write not yet fully implemented")
        return False
